        ...
        AMQP::Connection *connection, const char *data, size_t size)
{
    ...
    if (m_socket.is_open())
    {
        m_outputBuffer.push_back(std::vector<char>(data, size));
    ...
    {
        std::cerr << "No connection to the RabbitMQ" << std::endl;
        exit(1);
    }
}
...
{
    boost::asio::async_read(m_socket, boost::asio::buffer(m_asioInputBuffer),
